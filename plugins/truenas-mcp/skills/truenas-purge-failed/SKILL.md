...
`/mnt/<pool>/<app-name>/` — the operator decides manually whether to
wipe those.
...
Kept on host:
  /mnt/<pool>/<app_name>/   (operator decides manually)
```
...
- Delete host directories under `/mnt/`. The MCP server doesn't do
  this, full stop. The host filesystem is the operator's responsibility.
